...
* 속성
  - **.shape** : 배열의 차원별 크기를 튜플 형태로 반환합니다.
...
* 메소드
  - **ndarray.astype(dtype)** : 예시) `arr.astype(float32)`
    ...
    - **특징** : 원소의 타입을 지정한 dtype으로 변환합니다.
* 특징
  - 배열을 생성하고 반환합니다.
  - 타입은 `numpy.ndarray`입니다.
  - 배열의 원소들은 같은 타입으로 통일됩니다.
...
* 특징
  - 주어진 크기만큼 모든 원소의 값을 지정한 값으로 채워 생성하고 반환합니다.
...
* 특징
  - 대각선(Diagonal) 성분은 1이고 나머지 성분은 0으로 채워진 단위행렬(Identity Matrix)을 반환합니다.
...
* 매개변수
  ...
  - **step** : 값 사이의 간격입니다.
* 특징
  - start부터 stop(포함되지 않음) 직전까지 등차가 step인 배열을 생성하고 반환합니다.
...
* 특징
  - start부터 stop까지 num개의 원소를 균등한 간격으로 배열을 생성하고 반환합니다.

**numpy.logspace(start, stop, num, base)** : 예시) numpy.logspace(1, 10, 10, base=2)
* 매개변수
  - **start** : 로그스케일 시작 지점의 지수입니다.
  - **stop** : 로그스케일 끝 지점의 지수입니다.
  - **num** : 생성할 샘플(원소)의 총 개수입니다. (기본값: 50)
  - **base** : 로그의 밑(base) 값입니다. (기본값: 10.0)
* 특징
  - start부터 stop까지 num개의 로그스케일 원소를 균등한 간격으로 배열을 생성하고 반환합니다.

**numpy.random.normal()** : 예시) numpy.random.normal(0, 1, 10)
* 매개변수
  - **loc** : 정규 분포의 평균입니다. (기본값: 0.0)
  - **scale** : 정규 분포의 표준편차입니다. (기본값: 1.0, 음수 불가)
...
* 특징
  - 지정한 시드가 같은 경우 항상 동일한 난수 결과를 얻습니다. (의사 난수 생성 알고리즘 방식)
...
**1차원 배열 인덱싱** : 예시) arr[3]
* **개념** : 배열의 각 요소 자리마다 0부터 시작하는 인덱스를 부여하여 접근합니다.
...
* **특징**
  ...
  - **2차원 배열** : 행렬 곱셈 수행 결과를 반환합니다.

**수학 및 반올림 함수** : 예시) np.abs(arr), np.ceil(arr), np.floor(arr), np.round(arr), np.trunc(arr)
* **특징**
  - **`np.abs(arr)`** : 모든 요소의 절댓값을 반환합니다.
  - **`np.ceil(arr)`** : 올림 값을 반환합니다.
  - **`np.floor(arr)`** : 내림 값을 반환합니다.
  - **`np.round(arr)`** : 반올림 값을 반환합니다.
  - **`np.trunc(arr)`** : 소수점을 버립니다.
...
* **특징**
  - **비교 연산** : `==`, `>`, `<` 등을 통해 요소별 비교 후 불리언 배열 반환
  - **`np.array_equal(arr1, arr2)`** : 두 배열의 형태와 모든 원소가 완전히 같은지 확인
  - **삼각 함수** : `np.sin(arr)`, `np.cos(arr)`, `np.tan(arr)`
  - **`np.pi`** : 원주율 값 반환

**브로드캐스팅 (Broadcasting)** : 예시) arr1 + arr2
* **개념**
  * 크기가 서로 다른 배열 간에 연산이 가능하도록, 작은 배열의 형태를 자동으로 확장하여 연산하는 기법입니다.

**벡터 연산과 성능 (Vectorization)** : 예시) np.sum(arr), np.dot(arr1, arr2)
* **개념**
  * 파이썬의 일반 `for`문 대신 NumPy의 내부 최적화된 벡터 연산을 사용하여 수십 배 이상의 속도 향상과 성능 이점을 얻습니다.

**배열의 정렬 (Sort)** : 예시) np.sort(arr)
* **특징**
  - **`np.sort(arr)`** : 원소들을 오름차순으로 정렬한 새로운 배열을 반환합니다. (**원본 변경 없음**)
  - **내림차순 정렬** : `np.sort(arr)[::-1]` 슬라이싱 기법 활용
  - **2차원 배열 정렬** : `axis=1`(열 방향, 기본값), `axis=0`(행 방향), `axis=None`(전체 펼쳐서 정렬)
...
* **특징**
  - 전체 요소 개수에 맞게 형태를 변경한 새로운 배열을 반환합니다.
  - `-1`을 사용하여 크기를 자동으로 계산할 수 있습니다.

**ndarray.resize(shape)** : 예시) arr.resize(3, 4)
* **매개변수**
  - **shape** : 변경할 배열의 크기입니다.
* **특징**
  - 반환값이 아닌 **원본 배열 자체의 형태를 직접 변경**합니다.
...
* **매개변수**
  - **axis** : 크기가 1인 제거할 축의 위치입니다. (기본값: 크기가 1인 모든 차원 축소)
* **특징**
  - 배열의 형태 중 크기가 1인 차원을 제거하여 차원을 낮춥니다.

**전치 행렬 (Transpose)** : 예시) arr.T
* **특징**
  - 행과 열을 서로 맞바꾼 전치 행렬을 반환합니다.
...
* **매개변수**
  ...
  - **values** : 삽입할 값입니다.
  - **axis** : 삽입할 축의 방향입니다. (지정하지 않으면 1차원으로 평탄화된 후 삽입)
* **특징**
  - 지정한 위치에 새로운 값을 삽입한 새로운 배열을 반환합니다. (**원본 배열은 변경되지 않음**)

**numpy.delete(arr, index, axis)** : 예시) np.delete(arr, 2, axis=1)
* **매개변수**
  - **arr** : 대상 배열입니다.
  - **index** : 삭제할 위치의 인덱스입니다.
  - **axis** : 삭제할 축의 방향입니다. (지정하지 않으면 1차원으로 평탄화된 후 삭제)
* **특징**
  - 지정한 위치의 요소를 삭제한 새로운 배열을 반환합니다. (**원본 배열은 변경되지 않음**)

**numpy.append(arr1, values, axis)** : 예시) np.append(arr1, arr2, axis=0)
* **매개변수**
  - **arr1** : 대상 배열입니다.
  - **values** : 추가할 배열 또는 값입니다.
  - **axis** : 병합할 축의 방향입니다. (지정하지 않으면 1차원으로 평탄화된 뒤 끝에 추가)
* **특징**
  - 배열의 끝에 새로운 값이나 배열을 이어 붙인 새로운 배열을 반환합니다. (**원본 배열은 변경되지 않음**)

**numpy.vstack(tup)** : 예시) np.vstack((arr1, arr2))
* **매개변수**
  - **tup** : 수직 방향으로 결합할 배열들의 튜플 또는 리스트입니다.
* **특징**
  - 두 개 이상의 배열을 수직 방향(행 기준, `axis=0`)으로 쌓아서 병합합니다. (`np.append(..., axis=0)`과 동일)

**numpy.hstack(tup)** : 예시) np.hstack((arr1, arr2))
* **매개변수**
  - **tup** : 수평 방향으로 결합할 배열들의 튜플 또는 리스트입니다.
* **특징**
...
* **매개변수**
  - **tup** : 연결할 배열들의 튜플 또는 리스트입니다.
  - **axis** : 연결할 축의 방향입니다. (기본값: `0`)
* **특징**
...
* **매개변수**
  - **arr** : 분할할 대상 배열입니다.
  - **indices_or_sections** : 정수(균등 분할 개수) 또는 분할할 지점의 인덱스 리스트/튜플입니다.
* **특징**
