...
**슬라이싱 (Slicing)**
* **개념**
  * `[start:end:step]` 구문을 사용하여 지정한 범위의 요소를 잘라냅니다. (`end`는 포함되지 않습니다.)
* **주요 특징 및 예시**
...
> **펜시 인덱싱**
* **정의**
  * 정수 배열이나 리스트를 이용해 **비연속적이거나 불규칙한 위치의 요소들을 한 번에 지정하여 접근**하는 방식입니다.

---

**1차원 배열 펜시 인덱싱**
* **사용 예시**
  * `arr[[0, 2, 4]]` : 인덱스 0, 2, 4번에 해당하는 요소들(`[5, 15, 25]`)을 선택하여 배열로 반환합니다.

---

**2차원 배열 펜시 인덱싱 및 혼합 사용**
* **개념**
  * 펜시 인덱싱은 일반 인덱싱, 슬라이싱과 혼합하여 행과 열을 자유롭게 조합할 수 있습니다.
* **사용 예시**
  * `arr[[0, 2], 2:]` : 0번과 2번 행을 선택하고, 해당 행들의 2번 열 이후(`2:`)를 슬라이싱하여 추출합니다.
  * `arr[1:, [2, 3]]` : 1번 행부터 끝까지 슬라이싱하고, 해당 행들의 2번과 3번 열을 펜시 인덱싱으로 추출합니다.
